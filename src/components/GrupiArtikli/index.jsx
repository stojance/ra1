...
class GrupiArtikli extends React.Component {
    ...
    selectGrupaChange = (e) => {
        let newSelectedGrupa = Object.assign({},this.state.selectedGrupa, {tatko_ID: e.target.value});

        this.setState({selectedGrupa: newSelectedGrupa});
    }

    nazivGrupaChange = (e) => {
        let newSelectedGrupa = Object.assign({},this.state.selectedGrupa, {naziv: e.target.value});
    ...
    deleteClick = (e) => {
        ...
        if(window.confirm('СЛЕДУВА БРИШЕЊЕ !!! Дали сте сигурни ?')===false) return;
        this.setState({isLoaded: false});
        let data = {
            Grupa_ID: grupa.grupa_ID
    ...
    render () {
        ...
        if(selectedGrupa) {
            optionsGrupi = selectedGrupa.listGrupi.map(o => {
                return <option value={o.grupa_ID} key={o.grupa_ID}>{o.naziv}</option>;
              });
        }
        ...
        if (error) {
            ...
            return (
                <div className="container">
                    <span className="display-4">Групи Артикли</span>
                        ...
                        <div className="col-md-6">
                            {selectedGrupa? 
                                <div className="form-group row">
                                    <label className="control-label col-md-3">Група ID ({selectedGrupa.grupa_ID})</label>
                                    <input className="form-control form-control-sm col-md-7" 
                                        value={this.state.selectedGrupa.naziv}
                                        onChange={this.nazivGrupaChange}
                                        readOnly={selectedGrupa.tatko_ID===0}
                                    />
                                    <button className="btn btn-sm btn-info col-md-2" onClick={this.addClick}>Додади</button> 
                                </div>
                            : null}
                            {selectedGrupa && selectedGrupa.tatko_ID > 0 ? 
                                <div className="form-group row">
                                    <label className="control-label col-md-3">Татко</label>
                                    <select className="form-control form-control-sm col-md-9" 
                            ...
                            {selectedGrupa && selectedGrupa.tatko_ID > 0 ? 
                                <div className="form-group row">
                                    ...
                                    <button className="btn btn-sm btn-success col-md-3" 
                                        style={{width:"100%"}}
                                    ...
                                    <div className="col-md-3" />
                                    <button className="btn btn-sm btn-danger col-md-3" 
                                        style={{width:"100%"}}
